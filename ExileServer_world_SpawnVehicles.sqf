...
_mapsizeX = (worldSize - _gridSizeOffset);
_mapsizeY = (worldSize - _gridSizeOffset);
...
for "_xSize" from 0 to _mapsizeX step _gridSize do
{
	...
	for "_ySize" from 0 to _mapsizeY step _gridSize do
	{
		...
		{
			_positionReal = [_position, 0, _gridSizeOffset, 5, 0 , 1 , 0 , _spawnedPositions] call BIS_fnc_findSafePos;
